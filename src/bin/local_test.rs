...
async fn handle_kafka(action: KafkaAction) -> Result<()> {
    ...
    match action {
            ...
            match check_kafka_connection(&brokers).await? {
                true => {
                    ...
                    for broker in metadata.brokers() {
                        println!("   - {}:{} (id: {})", broker.host(), broker.port(), broker.id());
                    }
                    let topic_count = metadata.topics().iter().filter(|t| !t.name().starts_with("__")).count();
        ...
        KafkaAction::ListTopics => {
            ...
            let topics: Vec<_> = metadata.topics()
                .iter()
        ...
        KafkaAction::ClusterInfo => {
            ...
            let topics: Vec<_> = metadata.topics().iter().filter(|t| !t.name().starts_with("__")).collect();
            println!("\nTopics ({}):", topics.len());
            for topic in topics {
                println!("  - {} ({} partitions)", topic.name(), topic.partitions().len());
            }
...
async fn handle_whatsapp(action: WhatsappAction) -> Result<()> {
    use pdf_services::infrastructure::notifications::evolution_api::{
        EvolutionAPIClient, is_valid_dominican_phone
    };
    ...
    match action {
        ...
        WhatsappAction::SendText { phone, message } => {
            ...
            if !is_valid_dominican_phone(&phone) {
                println!("⚠️  Warning: '{}' may not be a valid Dominican phone number", phone);
