...
impl TypstGenerator {
    ...
    pub async fn generate_pdf(&self, typst_content: &str) -> Result<Vec<u8>> {
        ...
        let generated_visual_files = Self::extract_generated_visual_files(&typst_content);
        ...
        if !output.status.success() {
            ...
            self.cleanup_visual_files(&generated_visual_files).await;
        ...
        // Clean up only the QR/barcode PNG files generated for this render.
        self.cleanup_visual_files(&generated_visual_files).await;

        Ok(pdf_bytes)
    }

    fn extract_generated_visual_files(source: &str) -> Vec<PathBuf> {
        let mut files = Vec::new();

        for prefix in ["#image(\"qr_", "#image(\"barcode_"] {
            let mut rest = source;
            while let Some(start) = rest.find(prefix) {
                let image_arg_start = start + "#image(\"".len();
                let after_start = &rest[image_arg_start..];
    ...
    async fn cleanup_visual_files(&self, generated_files: &[PathBuf]) {
        for filename in generated_files {
            let _ = fs::remove_file(self.work_dir.join(filename)).await;
        }
    }
