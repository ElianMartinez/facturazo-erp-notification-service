...
impl TypstGenerator {
    ...
    pub async fn generate_pdf(&self, typst_content: &str) -> Result<Vec<u8>> {
        ...
        // Clean up QR code PNG files generated by templates
        self.cleanup_qr_files().await;

        Ok(pdf_bytes)
    }

    /// Clean up temporary QR code files generated by templates
    async fn cleanup_qr_files(&self) {
        if let Ok(mut entries) = fs::read_dir(&self.work_dir).await {
